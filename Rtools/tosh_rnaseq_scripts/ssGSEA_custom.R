...
################################
## Functions for converting gene ids between ensembl, gene symbol, entrez etc. and also between species

## Taken from here: https://github.com/broadinstitute/ssGSEA2.0 and formatted and customized to be used in R


## Set the gene database we are using (dont use whatever weird default)
# gene.set.databases <- "/Users/tosh/Desktop/Ruggles_Lab/code/ssgsea_test/ssgsea_db/h.all.v7.0.symbols.gmt"
# gct.dir <- "/Users/tosh/Desktop/Ruggles_Lab/code/ssgsea_test/ssgsea_input/"
# out.dir <- "/Users/tosh/Desktop/Ruggles_Lab/code/ssgsea_test/ssgsea_output/"
# gct.file <- "/Users/tosh/Desktop/Ruggles_Lab/projects/platelet-stress-mouse/output/rna_processing/normcounttab_v12.gct"
# 
# run_ssGSEA2(gct.dir = gct.dir, gene.set.databases = gene.set.databases, out.dir = out.dir)


run_ssGSEA2 <- function(gct.file, gene.set.databases, out.dir, plotrankplots = FALSE){
    ...
    spare.cores         = 4                   ## No. of cores to leave idle
    ...
    ## Hacking around this code
    # 1 - create a ssGSEA folder, and create the folders for input and output
    file.copy(from = gct.file, to = paste0(out.dir, basename(gct.file)), overwrite = TRUE)
    gct.dir <- out.dir

    ## identify all gct files
    # gct.files <- dir(gct.dir, pattern='\\.gct$', full.names=T)
    gct.files <- paste0(out.dir, basename(gct.file))
    names(gct.files) <- paste(  sub('\\.gct$', '', sub('.*/','', gct.files)), 'ssGSEA', sep='_' )
    
    ## import signature database - actually turns out GMT file into a list of genesets - pretty useful.
    ...
    for(i in names(gct.files)){
        ...
        ## run ssGSEA
        ## THIS IS STILL OUTPUTTING TO SOME WEIRD PLACE - the input directory.. followed by the date
        ## Sucks but i think we have to setdir to the out.dir
        keepwd <- getwd()
        setwd(out.dir)
        
        gsea.res <- ssGSEA2(input.ds = input.ds, 
                            gene.set.databases=gene.set.databases, 
                            sample.norm.type=sample.norm.type, 
                            weight=weight,statistic=statistic, 
        ...
        ## ssGSEA2 writes out the files, and the actual output from the function is not super useful...
        ...
        ## ADDING AN IF STATEMENT HERE BECAUSE THIS TAKES A LONG TIME AND MAKES A LOT OF STUFF AND WE DONT REALLY USE IT
        if (plotrankplots == TRUE) {
            ## create sub-folder
            dir.create('rank-plots', recursive = TRUE, showWarnings = FALSE)
    ...
    ## Looks like the thing that we really want here is the COMBINED.GCT file
    FOI <- paste0(out.dir, i, "-combined.gct")
    tempgct <- read.table(FOI, fill = TRUE, stringsAsFactors = FALSE)
    completetab <- tempgct[3:nrow(tempgct),]
    colnames(completetab) <- as.character(tempgct[2,])
    
    ## Its forcing a change directory - so I am going to change it back to whatever it was before we reset it
    setwd(keepwd)
    
    ## Now lets return the complete ssGSEA tab to use in other analyses
    return(completetab)
}
...
## #######################################################################################################
## FROM: https://github.com/broadinstitute/ssGSEA2.0
...
ssGSEA2 <- function (
    ...
    if(export.signat.gct){
        ...
        dir.create('signature_gct', recursive = TRUE, showWarnings = FALSE)
